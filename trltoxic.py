...
class ScriptArguments:
    ...
    model_name: Optional[str] = field(default="aamirmiy/Dialo_self-aware", metadata={"help": "the model name"})
...
# its own dataset.
def build_dataset(config, dataset_name="dfconvs.csv", input_min_text_length=2, input_max_text_length=8):
    tokenizer = AutoTokenizer.from_pretrained(config.model_name)
    tokenizer.pad_token = tokenizer.eos_token
    df = pd.read_csv(dataset_name)
    ds = Dataset.from_pandas(df)
    ds = ds.rename_columns({"Context": "review"})
    ds = ds.select_columns(["review", "Response"])
